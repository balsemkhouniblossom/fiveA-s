...
app.put("/api/patients/:id/images", upload.array("images", 10), async (req, res) => {
  ...
  try {
    ...
    patient.images = [...patient.images, ...imageFiles];

    const imagePath = path.join(__dirname, "public", "Images", imageFiles[0]);
    const formData = new FormData();
    formData.append("image", fs.createReadStream(imagePath));

    let diagnosticResult = "Non analysé";
    try {
      const flaskRes = await axios.post("http://127.0.0.1:5000/predict", formData, {
        headers: formData.getHeaders()
      });
      diagnosticResult = flaskRes.data.diagnostic; // "Normal" ou "Malade"
    } catch (err) {
      console.error("Erreur IA :", err.message);
    }

    if (!patient.diagnostics) patient.diagnostics = [];
    patient.diagnostics.push({ date: new Date(), result: diagnosticResult });
